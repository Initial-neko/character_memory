...
The repository now publishes a verified `uv.lock`, generated by `uv lock` and checked against `pyproject.toml` with `uv lock --check`. `scripts/sync-all.sh` already ran locked whenever the file was present; all three CI jobs now pass `--locked` as well, so a transitive dependency cannot drift out from under a green build. Change the lock deliberately with `uv lock` and commit it in the same PR as the dependency change.
