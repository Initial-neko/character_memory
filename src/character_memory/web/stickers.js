(() => {
  ...
  async function load({refresh = false} = {}) {
    ...
    cache = (data.stickers || []).map(item => ({
      ...item,
      pack_id: item.pack_id || "default",
      pack_name: item.pack_name || "内置",
      url: item.url || stickerAsset(item.id),
    }));
    return cache;
  }
  ...
  function wireFallbacks() {
    panel?.querySelectorAll(".sticker-choice img").forEach(img => {
  ...
  function render(stickers) {
    ...
    const tabs = allPacks.length > 1
      ? `<div class="sticker-pack-tabs">${allPacks.map(pack => `<button type="button" class="sticker-pack-tab${pack.id === active.id ? " active" : ""}" data-sticker-pack="${CM.escapeHtml(pack.id)}" title="${CM.escapeHtml(pack.name)}">${CM.escapeHtml(pack.name)}</button>`).join("")}</div>`
      : "";
  ...
  function showImportDialog(file) {
    ...
    panel.innerHTML = `<div class="sticker-import-card">
      ...
      <div class="sticker-import-file">${CM.escapeHtml(file.name)} · ${(file.size / 1024 / 1024).toFixed(1)} MiB</div>
      <div class="sticker-import-help">导入后所有人物和群聊都能使用。推荐 ZIP 内带 <code>all_tags.json</code> 或每组 <code>tags.json</code>；只有图片时可让 Vision 自动补标签。</div>
      <label class="sticker-auto-tag"><input type="checkbox" data-sticker-auto-tag checked> <span>AI 自动补标签 <small>只补缺失标签，不覆盖已有标注</small></span></label>
  ...
  async function importStickerFile() {
    ...
    if (!file.name.toLowerCase().endsWith(".zip")) {
      panel.innerHTML = '<div class="error">Web 导入使用 ZIP 格式。</div>';
      return;
    }
    if (file.size > 64 * 1024 * 1024) {
      panel.innerHTML = '<div class="error">表情包 ZIP 不能超过 64 MiB。</div>';
      return;
    }
