...
def attach_visual_routes(app):
    """Attach V0 camera/screen keyframe routes to the existing async runtime."""
    ...
    def accept_direct_visual_message(req: DirectVisualMessageRequest):
        ...
        logger.info(
            "visual.accept direct character=%s conversation=%s event_id=%s frames=%d sources=%s",
            req.character_id,
            req.conversation_id,
            event.id,
            len(frame_urls),
    ...
    def accept_group_visual_message(conversation_id: str, req: GroupVisualMessageRequest):
        ...
        logger.info(
            "visual.accept group conversation=%s event_id=%s frames=%d sources=%s mentions=%s",
            conversation_id,
            event.id,
            len(frame_urls),
            visual_metadata.get("sources"),
