...
DB_PATH = Path(os.environ.get("CHARACTER_MEMORY_E2E_DB", ROOT / ".e2e-character-memory.db"))
...
class DeterministicPersonModel(PersonModel):
    ...
    def _reaction(self, context: str, *, image: bool = False) -> PersonReaction:
        current = self._current_text(context)
        if "# Group Conversation Contract" in context:
            message = f"E2E group reply: {current}"
        elif image:
            message = f"E2E vision reply: {current}"
        elif "wake_reason" in context or "唤醒后的主动判断" in context or "时间自然过去了一段" in context:
    ...
    def react_with_images_for_session(self, context: str, image_data_urls: list[str], session_id: str) -> PersonReaction:
        return self._reaction(context, image=True)
