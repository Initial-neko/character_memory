...
async function showRuntime() {
  ...
  try {
    ...
    drawerBody.innerHTML = `<section class="section"><h3>Runtime 初始化耗时</h3>${timingHtml(data.runtime_init_timings)}</section><section class="section"><h3>Provider</h3><div class="kv"><div>Chat Model</div><div>${escapeHtml(p.chat_model)}</div><div>Embedding</div><div>${escapeHtml(`${p.embedding_provider} / ${p.embedding_model}`)}</div><div>Runtime Loaded</div><div>${escapeHtml(data.runtime_loaded)}</div></div></section><section class="section"><h3>Mental State</h3><pre>${escapeHtml(data.mental_state || "暂无")}</pre></section><section class="section"><h3>Persona</h3><pre>${escapeHtml(data.persona || "")}</pre></section><section class="section"><h3>Memory · 最近 ${data.memories.length} 条</h3><div class="card-list">${data.memories.map(m => `<div class="card"><strong>${escapeHtml(m.memory_type)}</strong><span> · ${escapeHtml(fmtTime(m.event_time))}</span><div>${escapeHtml(m.content)}</div></div>`).join("") || "<p>暂无 Memory。</p>"}</div></section><section class="section"><h3>Intent · 最近 ${data.intents.length} 条</h3><pre>${escapeHtml(JSON.stringify(data.intents, null, 2))}</pre></section>`;
  } catch (error) { drawerBody.innerHTML = `<div class="error">${escapeHtml(error.message)}</div>`; }
}
