...
def compile_context(persona: str, mental_state: str, memories, event, recent_events=None, last_chat_event=None) -> str:
    ...
    allowed = {
        EventType.USER_MESSAGE: "0~3 个 MESSAGE / EMOJI；也可以完全不回复",
        EventType.TIME_TICK: "0~3 个 MESSAGE / EMOJI；只有确实想主动表达时才发送",
        EventType.PROACTIVE_INTENT: "0~3 个 MESSAGE / EMOJI；也可以放弃或延后",
    }.get(event.event_type, "0~3 个 MESSAGE / EMOJI；也可以没有对外表达")
...
本事件允许的对外表达：{allowed}。
actions 是本轮真正对外发生的动作，最多 3 个；通常用 MESSAGE，单独的 emoji/颜文字可以用 EMOJI。自然需要连续两三条时可以拆开，但不要机械拆句或为了显得像人而刷屏。
如果确实没有想回复的内容，直接 actions=[]。不要为了礼貌、活跃度或“完成任务”硬补一句话。
不要无条件迎合，也不要为了提高互动率而主动联系。
内部结构可以稀疏，但对外表达不以“最短输出”为目标。严格服从 Persona 的表达习惯，自然使用标点、停顿、emoji 或颜文字；需要理解、好奇或关心时可以自然追问，但不要机械地用问题维持对话。
