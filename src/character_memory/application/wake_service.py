...
class CharacterWakeService:
    """Minimal process-local wake coordinator for direct characters.

    P0.17C deliberately keeps this small: periodic wake is only a roughly
    hourly opportunity to run the existing TIME_TICK cognition path. It is not
    a durable scheduler and it does not track user activity or group rooms.
    """
    ...
    def prime(self, now: datetime) -> None:
        """Start each character's first interval from process startup."""
        with self._lock:
    ...
    def wake(
        ...
        *,
        ...
        if not force and not self.is_due(character_id, at):
            return None

        # Mark before the provider call so a transient failure does not create a
        # retry storm on every 30-second poll. Manual wake intentionally bypasses
        # the interval via force=True.
